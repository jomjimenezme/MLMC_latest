...
void rademacher_create_PRECISION( level_struct* l, hutchinson_PRECISION_struct* h, int type, struct Thread* threading ) {
  if ( type == 0 ) {
    ...
    error0( "Unknown value for type of Rademacher vector in relation to level of creation\n" );
  }
}
...
gmres_PRECISION_struct* get_p_struct_PRECISION( level_struct* l ) {
  if ( l->depth == 0 ) {
    if ( strcmp( "PRECISION", "float" ) == 0 ) {
      ...
      return (gmres_PRECISION_struct*)&( g.p );
    }
  } else {
    return &( l->p_PRECISION );
  }
...
int apply_solver_PRECISION( level_struct* l, struct Thread* threading ) {
  ...
  if ( l->level > 0 ) {
    ...
    while ( 1 ) {
      ...
      if ( l->p_PRECISION.was_there_stagnation == 0 ) {
        break;
...
struct sample hutchinson_blind_PRECISION( level_struct* l, hutchinson_PRECISION_struct* h, int type, struct Thread* threading ) {
  ...
  for ( i = 0; i < h->max_iters[l->depth]; i++ ) {
    ...
    if ( i != 0 ) {
      ...
      START_MASTER( threading );
      if ( g.my_rank == 0 ) {
        printf( "[%d, trace: %e %c i%e, variance: %e] ", i, creal( trace ), ( cimag( trace ) < 0 ) ? '-' : '+', fabs( cimag( trace ) ), creal( variance ) );
      ...
      if ( i > h->min_iters[l->depth] && RMSD < cabs( trace ) * h->trace_tol * h->tol_per_level[l->depth] )
        break;
    }
  }
  double t1 = MPI_Wtime();
...
complex_PRECISION g5_3D_hutchinson_mlmc_difference_PRECISION( int type_appl, level_struct* l, hutchinson_PRECISION_struct* h, struct Thread* threading ) {
  ...
  for ( level_struct* l_tmp = finest_l; l_tmp != NULL; l_tmp = l_tmp->next_level ) {
    levels[lvl_nr] = l_tmp;
    if ( g.my_rank == 0 )
      printf( "Storing depth %d \t Function called at depth %d\n", l_tmp->depth, l->depth );
    if ( l_tmp == l )
      l_index = lvl_nr;
    lvl_nr++;
  }
