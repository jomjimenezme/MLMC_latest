...
void hopping_only_PRECISION_cpu( vector_PRECISION eta, vector_PRECISION phi, operator_PRECISION_struct *op, level_struct *l, struct Thread *threading )
{
  // finest level only
  if (l->depth != 0)
    error0("hopping_only_PRECISION_cpu(...) is supposed to be called from the finest level only.");
  ...
  //  this is the only difference with d_plus_clover: initialized to 0
  vector_PRECISION_define( eta, 0.0, start, end, l );
  ...
  // copy-paste neighbor coupling from d_plus_clover_PRECISION_cpu
  for ( i=start/2, phi_pt=phi+start; i<end/2; i+=6, phi_pt+=12 ) {
    prp_T_PRECISION( op->prnT+i, phi_pt );
    prp_Z_PRECISION( op->prnZ+i, phi_pt );
    prp_Y_PRECISION( op->prnY+i, phi_pt );
  ...
  for ( phi_pt=phi+start, end_pt=phi+end, D_pt = op->D+(start*3),
        nb_pt=neighbor+((start/12)*4); phi_pt<end_pt; phi_pt+=12 ) {
  ...
  for ( eta_pt=eta+start, end_pt=eta+end, D_pt = op->D+start*3,
        nb_pt=neighbor+(start/12)*4; eta_pt<end_pt; eta_pt+=12 ) {
